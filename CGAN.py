...
class CGAN(object):
    ...
    def __init__(self, sess, epoch, batch_size, z_dim, dataset_name, checkpoint_dir, result_dir, log_dir):
        ...
        if dataset_name == 'face' :
            ...
            self.y_dim = 100         # dimension of condition-vector (label)
            self.c_dim = 3
            ...
            self.data_X, self.data_y= load_train_data()
    ...
    def discriminator2(self, x, y, is_training=True, reuse=False):
    ...
    def discriminator(self, x, y, is_training=True, reuse=False):
        ...
        with tf.variable_scope("discriminator") as scope:
            ...
            net = tf.reshape(net, [self.batch_size, -1])
            net = lrelu(bn(linear(net, 1024, scope='d_fc3'), is_training=is_training, scope='d_bn3'))
            out_logit = linear(net, 3, scope='d_fc4')
            out = tf.nn.sigmoid(out_logit)
    ...
    def generator(self, z, y, is_training=True, reuse=False):
        ...
        with tf.variable_scope("generator", reuse=reuse):
            ...
            net = tf.nn.relu(bn(linear(net, 128 * 32 * 32, scope='g_fc2'), is_training=is_training, scope='g_bn2'))
            net = tf.reshape(net, [self.batch_size, 32, 32, 128])
            net = tf.nn.relu(
                bn(deconv2d(net, [self.batch_size, 64, 64, 64], 4, 4, 2, 2, name='g_dc3'), is_training=is_training,
                   scope='g_bn3'))

            out = tf.nn.sigmoid(deconv2d(net, [self.batch_size, 128, 128, 3], 4, 4, 2, 2, name='g_dc4'))
